...
def _scan_targets() -> tuple[list[tuple[Path, int, int]], list[Path]]:
    ...
    for d in get_dirs_to_reset():
        if is_protected(d):
            logger.warning(f"[DENIED] 拒绝处理受保护目录(配置可能有误): {d}")
            skipped.append(d)
            continue
...
def _print_plan(
    ...
    logger.info("")
    logger.info("[PROTECTED] 以下重要目录不会被删除:")
...
def _delete_one(
    ...
    if is_protected(path):
        logger.error(f"[{idx}/{total}] [DENIED] 删除前检查失败,跳过: {path}")
        return "受保护目录"
    ...
    try:
        ...
        logger.info(f"[{idx}/{total}] [OK] 已删除: {rel}")
        return None
    except OSError as e:
        logger.error(f"[{idx}/{total}] [FAIL] 删除失败 {rel}: {e}")
        return str(e)
...
def reset_project(yes: bool = False, force: bool = False, dry_run: bool = False) -> int:
    ...
    if not yes:
        logger.info("")
        if dry_run:
            logger.info("[TIP] 这是显式的 --dry-run。要真正执行删除,请加 --yes:")
        else:
            logger.info("[TIP] 这是 dry-run(默认行为)。要真正执行删除,请加 --yes:")
        logger.info("   python scripts/reset_project.py --yes")
        return 0

    if not force:
        if not _confirm(len(deletable)):
            logger.warning("[CANCEL] 用户取消,未执行删除")
            return 1
